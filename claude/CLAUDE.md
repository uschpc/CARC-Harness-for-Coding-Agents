# Claude Code on the CARC HPC cluster
...
enforced by managed permissions and a `PreToolUse` hook at `/etc/claude-code/`.
...
**The PreToolUse hook enforces this, it doesn't just advise it.** On a login
...
**Known Claude Code HPC issues.** Claude may exit on some interactive SLURM
sessions because shell-detection subprocesses consume stdin. On compute
nodes without a systemd user session (no `/run/user/$UID`) the Bash tool can
fail outright. If either happens, tell the user rather than silently
retrying — these are upstream bugs with open GitHub issues.
...
Writes to `~/.bashrc`, `~/.bash_profile`, `~/.zshrc`, `~/.ssh/**`,
`~/.aws/**`, `~/.config/gcloud/**`, `~/.netrc`, `~/.pgpass`, `~/.gnupg/**`,
`~/.globus/**`, and tool-config dirs (`~/.anthropic/`, `~/.codex/`,
`~/.openai/`, `~/.gemini/`) are set to `ask` — every attempt prompts and
there is no session-level bypass. On HPC, one bad `~/.bashrc` line takes
down the user's entire login environment.
...
Reading actual secrets (private keys, `~/.aws/credentials`, Globus tokens,
GPG private keys, credential stores for Anthropic/OpenAI/Codex/Gemini) is
hard-denied. Help the user debug auth by walking through `ssh -vvv`,
...
reading any of these — both via Claude's `Read` tool and via shell-outs
...
Do not suggest `--dangerously-skip-permissions`. Managed settings disable
it and training the user toward bypasses defeats the point of the hook.
