...
# could abort under 'set -u' before the hook reaches a block/ask decision.
...
print("\x1f".join([
    ...
    s(ti.get("file_path") or ti.get("notebook_path") or ti.get("path") or ti.get("filename")),
    s(d.get("session_id")),
    s(d.get("cwd")),
]))
' 2>/dev/null)
...
# 0. sudo / su as a command. Static permissions can catch literal forms, but
#    wrapped commands like 'bash -c "sudo ..."' or 'eval "su -"' need the
#    expanded-command check. Anchor to command position so 'echo sudo' and
#    substrings like 'git submodule' do not trip it.
...
# 2b. Recursive chmod/chown at the filesystem root '/'. Static command-prefix
#     patterns can miss real forms where the mode/owner appears before the
#     path, e.g. 'chmod -R 777 /'. Cover '/', and '/*', here.
...
#    /scratch1/<user>. Policy: a world-writable leftover dir in another lab
#    does NOT grant permission, and scratch dirs are per-user.
#
#    Treat a shared-storage path as a target only when it is an argument to a
#    destructive verb, redirect, dd 'of=', or rsync --delete. This avoids false
#    positives such as 'rm ./x; cat /project2/other/readme.md' while covering
#    destructive verbs beyond rm/chmod/chown/mv.
